 ...
 *
 * @package figuren-theater/security/disable_login_errors
 */
...
function load( string $username, WP_Error $error ) : void {
	...
	// cuurently returned errors from the WP_Error object
	...
	// only run
	// if new errors were silently merged into
	// what we asked
	if ( $errors_count === count( $error_types ) ) {

		// Add our filter to prevent theese error message 
